  ...
  <>
    ...
    <section className="bg-brand-black pt-32 pb-16">
      <div className="container">
        ...
        <h1 className="mt-3 font-display text-4xl text-brand-white sm:text-5xl">Our Services</h1>
    ...
    <section className="bg-brand-black pb-24">
      <div className="container grid gap-6 md:grid-cols-2">
        {SERVICES.map((s) => {
          ...
          return (
            <article key={s.slug} className="hover-card group rounded-lg border border-brand-gray bg-brand-black p-8">
              <div className="inline-flex h-12 w-12 items-center justify-center rounded-md bg-brand-gold/10 text-brand-gold">
                <Icon className="h-6 w-6" />
              </div>
              <h2 className="mt-6 font-display text-2xl text-brand-white">{s.name}</h2>
              <p className="mt-3 text-brand-white/80">{s.description}</p>
              <Link
                to={`/${s.slug}`}
